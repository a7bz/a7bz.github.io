    ...
    <Transition name="fade" mode="out-in">
        <div v-if="listData?.length" class="link-list">
            ...
            <div class="link-type-list">
                <div class="title">
                    <h2 class="name">
                        <span class="name-text">友链</span>
                        <span v-if="showCount" class="name-count">（{{ listData?.length || 0 }}）</span>
                    </h2>
                </div>
                <div class="all-link">
                    <a v-for="(link, index) in listData" :key="index" :class="['link-card', 's-card']" :href="link?.url"
                        ...
                        <div class="data">
                            <span :class="['name', { 'cf-friends-name': useFriendsLink }]">{{ link.name ||
                                link.title }}</span>
                            <span class="desc">{{ link.desc || link.description }}</span>
                        </div>
                    </a>
                </div>
            </div>
